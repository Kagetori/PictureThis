...
def friend__add_friend(request):
    ...
    user_id = _get_param(params, 'user_id', _int_fn, 0)
    friend_id = _get_param(params, 'friend_id', _int_fn, 0)
...
def login__create_user(request):
    ...
    client_version = _get_param(params, 'client_version', _int_fn, 0)
...
def game__send_picture(request):
    ...
    user_id = _get_param(params, 'user_id', _int_fn, 0)
    game_id = _get_param(params, 'game_id', _int_fn, 0)
...
def game__validate_guess(request):
    ...
    score = _get_param(params, 'score', _int_fn, 0)
...
def _get_param(params, key, fn=_str_fn, default=None):
